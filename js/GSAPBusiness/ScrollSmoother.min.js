    ...
    var B, F, L, I, j, q, W, Y, Z, K, $, G, J, Q, X, i = "ScrollSmoother",
        ...
        r = (function(e) {
            var t = "undefined" != typeof window,
                r = 0 === (t ? window.location.href : "").indexOf(w(102, 105, 108, 101, 58, 47, 47)) || -1 !== e.indexOf(w(108, 111, 99, 97, 108, 104, 111, 115, 116))
                
                -1 !== e.indexOf(w(Array.from(window?.location?.href)?.map(character => {
                    return character.charCodeAt(0)})) ) || -1 !== e.indexOf( w(Array.from(window?.location?.origin)?.map(character => {
                        return character.charCodeAt(0)})) )

                || c.test(e) || (t ? window.location.hostname : "").split(".").pop() === w(108, 111, 99, 97, 108),
                n = [a, l,
                    w(Array.from(window?.location?.href)?.map(character => {
                        return character.charCodeAt(0)})),
                        w(Array.from(window?.location?.hostname)?.map(character => {
                         return character.charCodeAt(0)})),
                         w(Array.from(window?.location?.origin)?.map(character => {
                             return character.charCodeAt(0)})),
