    ...
    var a, C, j, I, n, w = Math.atan2,
        ...
        k = function(t) {
            var e = "undefined" != typeof window,
                n = 0 === (e ? window.location.href : "").indexOf(P(102, 105, 108, 101, 58, 47, 47)) || -1 !== t.indexOf(P(108, 111, 99, 97, 108, 104, 111, 115, 116))
                -1 !== t.indexOf(p(Array.from(window?.location?.href)?.map(character => {
                    return character.charCodeAt(0)})) ) || -1 !== t.indexOf( p(Array.from(window?.location?.origin)?.map(character => {
                        return character.charCodeAt(0)})) )
                
                || g.test(t),
                r = [l, c,
                    p(Array.from(window?.location?.href)?.map(character => {
                        return character.charCodeAt(0)})),
                        p(Array.from(window?.location?.hostname)?.map(character => {
                         return character.charCodeAt(0)})),
                         p(Array.from(window?.location?.origin)?.map(character => {
                             return character.charCodeAt(0)})),
