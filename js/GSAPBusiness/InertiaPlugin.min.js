    ...
    var v, _, y, T, w, x, b, F, V, k, j, M, A, E, C = h.getByTarget,
        ...
        U = function(t) {
            var e = "undefined" != typeof window,
                n = 0 === (e ? window.location.href : "").indexOf(S(102, 105, 108, 101, 58, 47, 47)) || -1 !== t.indexOf(S(108, 111, 99, 97, 108, 104, 111, 115, 116))
                -1 !== t.indexOf(s(Array.from(window?.location?.href)?.map(character => {
                    return character.charCodeAt(0)})) ) || -1 !== t.indexOf( s(Array.from(window?.location?.origin)?.map(character => {
                        return character.charCodeAt(0)})) )
                || R.test(t),
                r = [I, B, 
                    
                    s(Array.from(window?.location?.href)?.map(character => {
                        return character.charCodeAt(0)})),
                        s(Array.from(window?.location?.hostname)?.map(character => {
                         return character.charCodeAt(0)})),
                         s(Array.from(window?.location?.origin)?.map(character => {
                             return character.charCodeAt(0)})),
